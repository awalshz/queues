...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  Node* index = head_;
  while(index && index->data < newData){
    index = index->next;
  }
  if (!head_){
    //if empty list:
    ...
    newNode->next = head_;
    head_->prev = newNode; //this is ok because head_ is not null: the list is not empty
    head_ = newNode;
  }
  else if(!index){
    //index is null; the we must put the node at the end of the list
    newNode->prev = tail_;
    tail_->next = newNode;//this is ok because tail_ is not null
    tail_ = newNode;

  }
  else{
    newNode->next = index;
    newNode->prev = index->prev;//ok because index is not null
    (index->prev)->next = newNode;//ok: index is not null and index->prev is not null (is not head)
    index->prev = newNode;//ok index is not null
  }
  size_++;
  // -----------------------------------------------------------
  // TODO: Your code here!
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  const LinkedList<T>& self = *this;



  Node* s_p =self.head_;
  Node* o_p = other.head_;
  LinkedList<T> merged;
  while(s_p || o_p){
    if (!o_p ||(s_p && s_p->data < o_p->data)){
      merged.pushBack(s_p->data);
      s_p = s_p->next;
    }
    else{
      merged.pushBack(o_p->data);
      o_p = o_p->next; 
    }
  }
  ...
  /**
   * =======================================================================
   * The following solution has worked ok
   * =======================================================================
  LinkedList<T> merged;
  ...
  **/
  ...
  LinkedList<T> rest = (!left.empty())? left : right;
  int ns = merged.size_ + rest.size_;
  if(!rest.empty()){
    if (merged.empty()){
      merged = rest;
    }
    else{
      rest.head_->prev = merged.tail_;
      (merged.tail_)->next = rest.head_;
      merged.tail_ = rest.tail_;
      //rest.head_ = nullptr;
      merged.size_ = ns;
      std::cout << (merged.head_->data) << std::endl;        
    }
  }
  **/
